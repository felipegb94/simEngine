...
arma::vec Solver::getPhi(std::vector<Body>* bodies,std::vector<c_constraint*> *constraints, double t){
	...
	for(std::vector<int>::size_type i = 0; i < constraints->size();i++){
		...
		if(type == "AbsoluteX"){
			std::cout << "it is an absolute x constraint" <<std::endl;
			...
			std::cout << "it is an absolute Y constraint" <<std::endl;
			...
			std::cout << "it is an absolute angle constraint" <<std::endl;
			...
			std::cout << "it is a revolute joint" <<std::endl;
			...
			temp.push_back(revJointPhi(0));
			temp.push_back(revJointPhi(1));
...
arma::mat Solver::getJacobian(std::vector<Body>* bodies,std::vector<c_constraint*> *constraints, double t){
	...
	arma::mat phi_q(bodies->size()*3,bodies->size()*3);
	...
	for(std::vector<int>::size_type i = 0; i < constraints->size();i++){
			...
			if(type == "AbsoluteX"){
				...
				int colFor1 = bodyID1 + 2*(bodyID1 -1) - 1;
				int colFor2 = bodyID2 + 2*(bodyID2 -1) - 1;
				int rowFor1 = colFor2;
				int rowFor2 = colFor1;
...
arma::vec Solver::getNu(std::vector<Body>* bodies,std::vector<c_constraint*> *constraints, double t){
	...
	for(std::vector<int>::size_type i = 0; i < constraints->size();i++){
		...
		if(type == "AbsoluteX"){
			...
			temp.push_back(revJointNu(0));
			temp.push_back(revJointNu(1));
		}
	}
	nu = temp;
...
arma::vec Solver::getGamma(std::vector<Body>* bodies,std::vector<c_constraint*> *constraints, double t){
	...
	for(std::vector<int>::size_type i = 0; i < constraints->size();i++){
		...
		if(type == "AbsoluteX"){
			...
			temp.push_back(revJointGamma(0));
			temp.push_back(revJointGamma(1));
		}
	}
	gamma = temp;
	return gamma;
...
double Solver::getAbsX(c_absX* absX, Body body,double t, int flags){
	std::cout << "getting absoluteX" << std::endl;
	double xComponent;
...
double Solver::getAbsY(c_absY* absY, Body body,double t,int flags){
	...
	std::cout << "getting absoluteY" << std::endl;
	double yComponent;
...
double Solver::getAbsAngle(c_absAngle* absAngle, Body body,double t,int flags){
	...
	std::cout << angleComponent << std::endl;
...
arma::vec Solver::getRevJoint(c_revJoint* revoluteJoint, Body body1, Body body2,double t, int flags){
	...
	std::cout << revoluteComponents << std::endl;
...
arma::rowvec Solver::getAbsAngle_jac(c_absAngle* absAngle, Body body,double t){
	...
	angleComponents(0) = 0;
	angleComponents(1) = 0;
	angleComponents(2) = 1 - absAngle->c_dFunction.eval(t);
	std::cout << angleComponents << std::endl;
